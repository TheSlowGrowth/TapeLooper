...
extern const float pitchRatioHigh[257];
extern const float pitchRatioLow[257];

MANUAL_INLINE float semitonesToRatio(float semitones)
{
    float pitch = semitones + 128.0f;
    int32_t integral = int32_t(pitch);
    float fractional = pitch - float(integral);

    return pitchRatioHigh[integral] * pitchRatioLow[int32_t(fractional * 256.0f)];
}